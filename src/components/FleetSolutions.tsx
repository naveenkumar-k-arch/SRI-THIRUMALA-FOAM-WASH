          ...
          {filterTabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`px-4 sm:px-5 py-2.5 rounded-xl font-bold text-xs sm:text-sm transition-all duration-200 cursor-pointer flex items-center gap-2 ${
                  isSelected
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredVehicles.map((veh) => {
            ...
            return (
              <div
                key={veh.id}
                className="group rounded-3xl bg-white/[0.03] hover:bg-white/[0.07] border border-white/10 hover:border-red-400/40 transition-all duration-300 overflow-hidden flex flex-col justify-between shadow-xl"
              >
                <div>
                  {/* Vehicle Image */}
                  <div className="relative h-44 w-full overflow-hidden bg-slate-900">
                    ...
                    <div className="absolute top-3.5 left-3.5">
                      <span className="text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full bg-black/70 backdrop-blur-md border border-white/20 text-amber-300 flex items-center gap-1.5 shadow-md">
                        <Icon className="w-3 h-3 text-orange-400" />
                        <span>{veh.categoryLabel}</span>
                      </span>
                    </div>
                  </div>
                  ...
                  {/* Card Content */}
                  <div className="p-5">
                    <h3 className="text-lg font-black text-white font-['Outfit'] group-hover:text-amber-300 transition-colors">
                      {veh.name}
                    </h3>
                ...
                <div className="p-5 pt-0">
                  <div className="pt-3 border-t border-white/10 flex items-center justify-between text-[11px] font-bold text-slate-400">
                    ...
                    <span className="text-orange-400">Snow Foam</span>
                  </div>
                </div>
              </div>
            );
